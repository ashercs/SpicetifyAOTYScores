      ...
      if (DupeAristArray.length > 1) {
        for (let a = 0; a < DupeAristArray.length; a++) {
          if (a !== DupeAristArray.length - 1) {
            SimToAlbumJSON += `"${similarity(DupeAristArray[a], album2)}": "${DupeAristArray[a].replaceAll('"', '\\"')}",
`;
          }
          if (a == DupeAristArray.length - 1) {
            SimToAlbumJSON += `"${similarity(DupeAristArray[a], album2)}": "${DupeAristArray[a].replaceAll('"', '\\"')}"
          ...
          DupeArtistSimArr.push(similarity(DupeAristArray[a], album2));
        }
      }
      if (DupeAristArray.length >= 1) {
  ...
  async function update2() {
    var _a2, _b;
    if (!Spicetify.Player.data.playbackId || !((_b = (_a2 = Spicetify.Player.data) == null ? void 0 : _a2.track) == null ? void 0 : _b.metadata))
      return;
